...
# The POSIX bootstrap installs these before handing control to Zsh.
for prerequisite in git curl stow zsh; do
    if ! command -v "$prerequisite" >/dev/null 2>&1; then
        echo "❌ Bootstrap prerequisite is unavailable: $prerequisite" >&2
        echo "   Rerun ./setup.sh to install missing prerequisites." >&2
        exit 1
    fi
done
...
# Zsh plugins are plain Git checkouts pinned in a shared TOML manifest.
